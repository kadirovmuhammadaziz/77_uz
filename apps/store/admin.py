...
class AdAdmin(TranslationAdmin):
    list_display = [
        'name', 'category', 'seller', 'price_formatted',
        'status', 'is_top', 'view_count', 'published_at'
    ]
    list_filter = [
        'status', 'is_top', 'category', 'region',
        'published_at', 'created_at'
    ]
    search_fields = [
        'name_uz', 'name_ru', 'description_uz', 'description_ru',
        'seller__first_name', 'seller__last_name', 'seller__phone'
    ]
    prepopulated_fields = {'slug': ('name_uz',)}
    ...
    fieldsets = (
        ('Asosiy ma\'lumotlar', {
            'fields': ('name_uz', 'name_ru', 'slug', 'category', 'description_uz', 'description_ru')
        }),
        ('Narx va joylashuv', {
            'fields': ('price', 'region', 'district', 'address')
        }),
        ('Sotuvchi', {
            'fields': ('seller',)
        }),
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'category', 'seller', 'region', 'district'
        )
...
class FavoriteProductAdmin(admin.ModelAdmin):
    list_display = ['user_or_device', 'ad', 'created_at']
    list_filter = ['created_at']
    search_fields = [
        'user__first_name', 'user__last_name', 'user__phone',
        'device_id', 'ad__name_uz', 'ad__name_ru'
    ]
...
class SavedSearchAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'search_query', 'category', 'region',
        'price_range', 'created_at'
    ]
    list_filter = ['category', 'region', 'created_at']
    search_fields = [
        'user__first_name', 'user__last_name', 'search_query'
    ]
...
class PopularSearchAdmin(TranslationAdmin):
    list_display = ['name', 'search_count', 'is_active', 'updated_at']
